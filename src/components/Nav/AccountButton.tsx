...
const AccountButton: React.FC<AccountButtonProps> = ({ text }) => {
  ...
  return (
    <div>
      {!account ? (
        <Button className='btn-wallet' onClick={handleWalletProviderOpen} color="primary" variant="contained">
          {buttonText}
        </Button>
      ) : (
        <Button className='btn-wallet' variant="contained" onClick={onPresentAccountModal}>
          My Wallet
        </Button>
      )}
